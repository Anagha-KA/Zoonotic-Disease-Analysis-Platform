...
# Load Your ML Model
xgboost_model = joblib.load("xgboost_model.pkl")   # <-- Make sure file exists!
...
# -----------------------------
def prepare_ml_features(ct_values):
    """
    Converts extracted Ct values to a flat numeric feature dictionary
    for XGBoost prediction.
    Missing values are filled with -1.
    """
    features = {}

    for gene, value in ct_values.items():
        gene_name = gene.replace(" ", "_").lower()

        try:
            features[gene_name] = float(value)
        except:
            features[gene_name] = -1  # Missing Ct
...
def ct_to_risk(ct_vals):
    numeric_cts = [
        float(v) for v in ct_vals.values() 
...
def upload_file():
    ...
    try:
        ...
        # -------------------------
        # Regex: Disease + Result
        # -------------------------
        disease_match = re.search(r"(Dengue|Nipah|Rabies|Zoonotic)", text, re.IGNORECASE)
        result_match = re.search(r"Overall result:\s*(Positive|Negative|Detected|Not Detected)",
                                 text, re.IGNORECASE)
        ...
        # Format 1 - NS1 gene (Ct = 22.3)
        format1 = re.findall(r"([A-Za-z0-9]+ gene).?\(Ct\s*=?\s*([\d.]+)\)", text, re.IGNORECASE)

        # Format 2 - E gene Detected 18.2
        format2 = re.findall(
            r"(NS1 gene|E gene|N gene|G gene)\s+(Detected|Positive|Negative)\s*([\d.]+)?",
            text,
            re.IGNORECASE
        )

        for gene, value in format1:
            ct_values[gene.strip()] = value

        for gene, _, value in format2:
            if value:
                ct_values[gene.strip()] = value
        ...
        # XGBOOST PREDICTION
        # ------------------------------------------------
        ml_features = prepare_ml_features(ct_values)
        df_features = pd.DataFrame([ml_features])

        try:
            ml_pred = xgb_model.predict(df_features)[0]

            if hasattr(xgb_model, "predict_proba"):
                prob = float(max(xgb_model.predict_proba(df_features)[0]))
            else:
                prob = None
        except:
            ml_pred = "Model Error"
        ...
        # ------------------------------------------------
        rule_suggestion = dynamic_suggestions(
            disease=disease,
        ...
        # ------------------------------------------------
        # LLM SUGGESTION (YOU CAN REPLACE WITH YOUR MODEL)
        # ------------------------------------------------
        llm_suggestion = (
            f"Based on the disease: {disease} and the risk level: {risk_level}, "
            f"ensure proper hydration, monitor symptoms, and consult your doctor immediately "
            f"if conditions worsen."
        )

        # ------------------------------------------------
        # PREPARE REPORT ENTRY FOR DB
        # ------------------------------------------------
        report_entry = {
            "user_id": user_id,
            "disease": disease,
